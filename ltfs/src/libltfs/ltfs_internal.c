...
int ltfs_index_alloc(struct ltfs_index **index, struct ltfs_volume *vol)
{
	...
	if (ret) {
		ltfsmsg(LTFS_ERR, "11166E", ret);
		ltfs_mutex_destroy(&newindex->dirty_lock);
		if (newindex) {
			free(newindex);
			newindex = NULL;
		}
	...
	if (ret) {
		ltfsmsg(LTFS_ERR, "11166E", ret);
		ltfs_mutex_destroy(&newindex->dirty_lock);
		ltfs_mutex_destroy(&newindex->refcount_lock);
		if (newindex) {
			free(newindex);
			newindex = NULL;
		}
	...
	tape_block_t part_eod, struct ltfs_volume *vol)
{
	...
	dcache_enabled = dcache_initialized(NULL);
...
#endif /* 0 */
	buf = malloc(vol->label->blocksize);
	if (! buf) {
...
int ltfs_check_medium(bool fix, bool deep, bool recover_extra, bool recover_symlink, struct ltfs_volume *vol)
{
	...
	check_err(ltfs_seek_index(vol->label->partid_ip, &ip_eod, &ip_endofidx, &ip_fm_after,
		&ip_blocks_after, recover_symlink, vol), "11213E", out_unlock);
	ip_have_index = (ret == 0);
	if (ip_have_index) {
		ip_index = vol->index;
		vol->index = NULL;
	...
	check_err(ltfs_seek_index(vol->label->partid_dp, &dp_eod, &dp_endofidx, &dp_fm_after,
		&dp_blocks_after, recover_symlink, vol), "11214E", out_unlock);
	dp_have_index = (ret == 0);
	if (dp_have_index) {
		dp_index = vol->index;
		vol->index = NULL;
...
int ltfs_split_symlink( struct ltfs_volume *vol )
{
	...
	if ( ret==-LTFS_NO_DENTRY  ) {
		ret = ltfs_fsops_create( lfdir, true, false, &workd, vol);
		if ( ret<0 ) {
			free(lfdir);
			return ret;
		} else {
	...
	for( i=0; i<(vol->index->symerr_count); i++ ){
		...
		while( next_tok ){
			...
			if( !basedir ) {
				ret = ltfs_fsops_create( path, true, false, &workd, vol);
				if ( ret<0 )
					goto err_out_func;
